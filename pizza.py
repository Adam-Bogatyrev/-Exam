class Pizza:
    def __init__(self, name: str, dough: str, sauce: str, toppings: str, price: str) -> None:
        self.name = name
        self.dough = dough
        self.sauce = sauce
    ...
    def prepare(self) -> None:    
        print(
            f"Пдготовка {self.name} c тестом {self.dough}, соусом {self.sauce} и начинкой {self.toppings}"
        ) 
    ...
    def cut(self) -> None:
        print(
            f"C нарезками {self.dough}, {self.toppings}"
        )
...
class Terminal:
    ...
    def take_order(self) -> None:
        ...
        while True:
            ...
            if int(choice) in self.menu:
                ...
                print("Не верный набор. Поалуйста, выберите пиццу из меню")
                continue
    ...
    def take_payment(self) -> None:
        if self.order:
            print("Оплата принята. Ваш заказ готовиться.")
